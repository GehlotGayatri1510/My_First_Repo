...
const PageList_VendorMaster = () => {
  const [gridData, setGridData] = useState([]);
  const [loading, setLoading] = useState(true);
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const breadCrumbTitle = "Vendor Master";
  const breadcrumbItem = "Vendor Master";
  const rtPage_Add = "/AddVendor";
  const rtPage_Edit = "/EditVendor";
  ...
  const columns = useMemo(
    () => [
       ...
       
      {
        ...
        accessor: "Status",
        Cell: ({ value }) => (
          <span style={{ color: value === true ? "green" : "red", fontWeight: "bold" }}>
...
export default PageList_VendorMaster;
