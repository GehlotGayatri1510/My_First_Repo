...
const PageList_Uploads = () => {
  const [gridData, setGridData] = useState([]);
  const [loading, setLoading] = useState(true);
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const breadCrumbTitle = "Files Master";
  const breadcrumbItem = "Files Master";
  const rtPage_Add = "/AddFiles";
   
  const API_URL = API_WEB_URLS.MASTER + "/0/token/FilesMaster";
  const API_URL1 = API_WEB_URLS.MASTER + "/0/token/FileMaster";
  ...
  const btnDeleteOnClick = (Id) => {
      Fn_DeleteData(dispatch, setState, Id, `${API_URL1}`, true)
    };
...
export default PageList_Uploads;
