...
const ReceivedEMDReport = () => {
  const [gridData, setGridData] = useState([]);
  const [loading, setLoading] = useState(true);
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const breadCrumbTitle = "Received EMD Report";
  const breadcrumbItem = "Received EMD Report";
  const API_URL = `${API_WEB_URLS.MASTER}/0/token/ReceivedEMDReport`;
...
export default ReceivedEMDReport;
