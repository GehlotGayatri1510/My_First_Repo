...
const SidebarContent = (props) => {
  ...
  (SidebarData || [])
    .filter((item) => {
      if (obj && obj.F_UserType === 1) {
        return ["Masters", "Tender Form"].includes(item.label);
      } else {
        return !["Masters", "Tender Form"].includes(item.label);
      }
    })
      ...
      <React.Fragment key={key}>
        {item.isMainMenu ? (
          <li
            ...
            style={{
              // backgroundColor: key % 2 === 0 ? "#e6f3ff" : "#e6f3ff", // Even: Light Blue, Odd: Light Orange
              backgroundColor: key % 2 === 0 ? "#e6f3ff" : "#e6f3ff", 
          ...
          >
            ...
            >
              ...
              <span className="menu-item" style={{color:'black'}}>{props.t(item.label)}</span>
              {item.issubMenubadge && (
                <span
